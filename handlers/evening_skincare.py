...
class SkincareStatesEvening(StatesGroup):
...
def make_router(bot: Bot) -> Router:
    ...
    async def start_skincare_selection(message: types.Message, state: FSMContext):
        ...
        await message.answer(
            "What skin products did you use this evening?", reply_markup=skincare_kb()
        )
        await state.set_state(SkincareStatesEvening.choosing_skincare)

    @router.message(SkincareStatesEvening.choosing_skincare)
    async def select_skincare_product(message: types.Message, state: FSMContext):
        ...
        if message.text == "/done":
            ...
            await state.set_state(SkincareStatesEvening.save_data_in_db)
    ...
    @router.message(SkincareStatesEvening.save_data_in_db, F.text.casefold() == "yes")
    async def process_final_decision3(message: Message, state: FSMContext):
        print(f"{__file__}")
        await save_data(message, state)

    @router.message(
        SkincareStatesEvening.save_data_in_db, F.text.casefold() == "change"
    )
...
async def save_data(message: Message, state: FSMContext):
    ...
    evening_skincare_products_str = ", ".join(user_data.get("chosen_skincare", []))

    with Session() as session:
        try:
            # session.begin()
            user = session.query(User).filter_by(chat_id=message.from_user.id).first()
            user_answer = EveningSkincare(
                user_id=user.id,
                evening_skincare=evening_skincare_products_str,
                date=datetime.today(),
            )
